...
const th = { color: '#334155', borderBottom: '1px solid rgba(255,255,255,0.06)', fontSize: '0.6rem', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase', py: 1.2, px: 1.5, fontFamily: 'Inter', background: 'rgba(0,0,0,0.2)', whiteSpace: 'nowrap' }
const td = { color: '#94a3b8', borderBottom: '1px solid rgba(255,255,255,0.04)', fontSize: '0.78rem', py: 1.2, px: 1.5, fontFamily: 'Inter' }
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      const [flagRes, corrRes, statRes] = await Promise.all([
        fetch('/api/corrections?type=flagged&status=pending').then(r => r.json()),
        fetch('/api/corrections?type=corrections').then(r => r.json()),
        fetch('/api/corrections?type=stats').then(r => r.json()),
      ])
        ...
        <Box>
          ...
          <Box display="flex" justifyContent="space-between" alignItems="flex-start" mb={3}>
            <Box>
              <Box display="flex" alignItems="center" gap={1} mb={0.3}>
                ...
                <Typography sx={{ fontSize: '1.2rem', fontWeight: 700, color: '#f1f5f9', fontFamily: 'Inter', letterSpacing: '-0.3px' }}>
                  Auto-Correction System
                </Typography>
              </Box>
              <Typography sx={{ fontSize: '0.68rem', color: '#475569', fontFamily: 'Inter' }}>
                Review flagged values · Approve corrections · System learns and auto-fixes future uploads
              </Typography>
            </Box>
            <Button onClick={loadData} size="small" sx={{ color: '#64748b', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '8px', fontFamily: 'Inter', fontSize: '0.72rem' }}>Refresh</Button>
          </Box>
              ...
              {[
                ...
                <Grid item xs={6} md={3} key={i}>
                  <Box sx={{ p: 2.2, borderRadius: '12px', background: '#111827', border: `1px solid ${s.color}18` }}>
                    ...
                    <Typography sx={{ fontSize: '0.72rem', fontWeight: 600, color: '#f1f5f9', fontFamily: 'Inter' }}>{s.label}</Typography>
                    <Typography sx={{ fontSize: '0.6rem', color: '#475569', fontFamily: 'Inter' }}>{s.sub}</Typography>
                  </Box>
                </Grid>
              ))}
              {stats.latestUpload && (
                <Grid item xs={12}>
                  <Box sx={{ p: 2.2, borderRadius: '12px', background: '#111827', border: '1px solid rgba(34,197,94,0.18)' }}>
                    <Typography sx={{ fontWeight: 700, fontSize: '0.84rem', color: '#f1f5f9', mb: 0.3, fontFamily: 'Inter' }}>Last Upload Summary</Typography>
                    <Typography sx={{ fontSize: '0.64rem', color: '#64748b', mb: 1.5, fontFamily: 'Inter' }}>
            ...
            {[
              ...
              <Box key={i} display="flex" alignItems="center" gap={1}>
                ...
                <Box>
                  ...
                  <Typography sx={{ fontSize: '0.6rem', color: '#64748b', fontFamily: 'Inter' }}>{s.desc}</Typography>
                </Box>
                {i < 4 && <Typography sx={{ color: '#334155', ml: 1, fontSize: '0.8rem' }}>→</Typography>}
              </Box>
            ))}
          </Box>
          ...
          <Box sx={{ borderBottom: '1px solid rgba(255,255,255,0.07)', mb: 2.5 }}>
            <Tabs value={tab} onChange={(e, v) => setTab(v)}
              sx={{ '& .MuiTab-root': { color: '#475569', fontFamily: 'Inter', fontSize: '0.8rem', textTransform: 'none', minWidth: 0, mr: 3 }, '& .Mui-selected': { color: '#f1f5f9' }, '& .MuiTabs-indicator': { background: '#3b82f6' } }}>
          ...
          {loading ? (
            ...
            <Box>
              ...
              <Box display="flex" gap={1.5} mb={2} alignItems="center">
                <Typography sx={{ fontSize: '0.72rem', color: '#475569', fontFamily: 'Inter' }}>Filter by field:</Typography>
              ...
              {filtered.length === 0 ? (
                <Box sx={{ p: 4, borderRadius: '12px', background: '#111827', border: '1px solid rgba(255,255,255,0.07)', textAlign: 'center' }}>
                  ...
                  <Typography sx={{ fontSize: '0.72rem', color: '#475569', fontFamily: 'Inter', mt: 0.5 }}>Upload new data to check for issues.</Typography>
                </Box>
              ) : (
                <Box sx={{ p: 2.5, borderRadius: '12px', background: '#111827', border: '1px solid rgba(255,255,255,0.07)' }}>
                  <Typography sx={{ fontWeight: 600, fontSize: '0.82rem', color: '#f1f5f9', fontFamily: 'Inter', mb: 0.3 }}>
                    {filtered.length} values need your decision
                  </Typography>
                  <Typography sx={{ fontSize: '0.62rem', color: '#475569', fontFamily: 'Inter', mb: 2 }}>
                                ...
                                {item.suggested_value ? (
                                  <Box>
                                    ...
                                    <LinearProgress variant="determinate" value={conf}
                                      sx={{ height: 3, borderRadius: 2, background: 'rgba(255,255,255,0.07)', '& .MuiLinearProgress-bar': { background: cc } }} />
                                  </Box>
                                ) : (
                                  <Typography sx={{ fontSize: '0.65rem', color: '#ef4444', fontFamily: 'Inter' }}>No match found</Typography>
                                )}
                              </TableCell>
                              <TableCell sx={{ ...td, fontFamily: "'JetBrains Mono'", color: '#64748b' }}>
                                {item.occurrences}×
                              </TableCell>
                                  ...
                                  <Box display="flex" gap={0.8}>
                                    ...
                                    <Tooltip title="Ignore for now">
                                      <IconButton onClick={() => act(item.id, 'ignore', item)} size="small"
                                        sx={{ color: '#475569', background: 'rgba(255,255,255,0.04)', borderRadius: '7px', p: 0.6, '&:hover': { background: 'rgba(255,255,255,0.08)' } }}>
                    ...
                    <TableBody>
                      {corrections.map(c => (
                        <TableRow key={c.id} sx={{ '&:hover': { background: 'rgba(255,255,255,0.02)' } }}>
                          ...
                          <TableCell sx={td}>
                            <Chip label={c.method} size="small" sx={{ height: 16, fontSize: '0.55rem', fontWeight: 600, background: 'rgba(255,255,255,0.05)', color: '#64748b', border: '1px solid rgba(255,255,255,0.07)' }} />
                          </TableCell>
                          ...
                          <TableCell sx={{ borderBottom: '1px solid rgba(255,255,255,0.04)', py: 1.2, px: 1.5 }}>
                            <Tooltip title="Delete this rule">
                              <IconButton onClick={() => deleteCorrection(c.id)} size="small"
                                sx={{ color: '#334155', p: 0.5, '&:hover': { color: '#ef4444', background: 'rgba(239,68,68,0.08)' } }}>
